...
| `analyze` after every `tap`/`input` (or always `--no-observe` then re-analyze) | Let the action return `observation` (default, already compact); narrow it with `--observe-fields`, and add `--until "<predicate>"` when the next screen is slow | Post-action observation already has fresh ids. Extra analyzes double round trips. Measured on a 5-scenario run: 37 taps became 73 `analyze` + 37 `wait` calls this way, roughly 60% of wall-clock spent on avoidable round trips. |
