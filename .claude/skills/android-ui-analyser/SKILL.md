...
14. **Wait on state, never sleep.** `aua wait --for "<text>"` waits for text to appear; `aua wait --for-stable` returns once the screen stops visually changing (grid pixel-hash; looping spinners/video are auto-masked so they don't block). Prefer goal waits over `--for-stable` after tabs/taps; never fixed sleeps.
