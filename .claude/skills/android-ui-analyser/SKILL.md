...
9. **Act, then read the screen the action gives back.** IDs are only valid until the screen changes. By default every state-changing action (tap/input/swipe/scroll-to/key) returns the next screen inline in `observation` (elements with fresh ids) — so you rarely need a separate `analyze`: `type → tap send` is two calls, not three, and `goto` returns the destination's `elements` too. Pass `--no-observe` to skip it on action-only sequences. `observation` is a no-wait snapshot taken right after the action — use a plain `analyze` (after `wait --for-stable`) when the screen is still animating.
10. **Wait on state, never sleep.** `aua wait --for "<text>"` waits for text to appear; `aua wait --for-stable` returns once the screen stops visually changing (cheap perceptual-hash over screenshots — ideal for image generation / loading / video, works on opaque screens). Prefer these to fixed sleeps.
11. **Stop the daemon when done.** `aua daemon stop` releases the warm connection.
