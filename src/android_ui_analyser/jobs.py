...
def worker_is_busy(cache_dir: str | Path, pid: int) -> bool:
    """Whether process *pid* is still running a background job recorded under *cache_dir*.

    A job worker is a thread inside the warm daemon (``worker_pid`` is the daemon's own pid),
    so signalling that process abandons the wait mid-flight. Anything that considers replacing
    a daemon it did not start has to ask this first.

    Deliberately unfiltered by owner, unlike :meth:`JobManager.list`: the whole point is to see
    another agent's in-flight work, and that agent's jobs are invisible to this process's owner.
    Reads the persisted records rather than asking the daemon, so it also works against a
    daemon too old to answer a new command — which, under version skew, is every one of them.
    """
    if not _pid_alive(pid):
        return False
    for path in _job_dir(cache_dir).glob("*.json"):
        try:
            state = JobState.model_validate_json(path.read_text(encoding="utf-8"))
        except (OSError, ValueError, TypeError):
            continue
        if state.status not in _TERMINAL and state.worker_pid == pid:
            return True
    return False
