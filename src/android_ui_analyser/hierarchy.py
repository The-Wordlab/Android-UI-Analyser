...
def parse_hierarchy(xml: str, screen_size: tuple[int, int] | None = None) -> list[Element]:
    ...
    def visit(node: Any, actionable_ancestor: bool, parent_slot: int | None = None) -> None:
        ...
        # Passed to Element by name rather than splatted from a dict: a `**dict[str, bool | None]`
        # cannot be matched against the individual fields, so every one of these six went to
        # `Element` unchecked and mypy blamed whichever positional field it landed on.
        tri_checkable = _tri(node, "checkable")
        tri_checked = _tri(node, "checked")
        tri_selected = _tri(node, "selected")
        tri_scrollable = _tri(node, "scrollable")
        tri_long_clickable = _tri(node, "long-clickable")
        tri_password = _tri(node, "password")
            ...
            collected.append(
                (
                    ...
                    Element(
                        ...
                        checkable=tri_checkable,
                        checked=tri_checked,
                        selected=tri_selected,
                        scrollable=tri_scrollable,
                        long_clickable=tri_long_clickable,
                        password=tri_password,
                    ),
                    parent_slot,
                )
            )
    ...
    def _renumbered(new_id: int, slot: int) -> Element:
        # The parent slot is bound to a name first: narrowing `collected[slot][2]` inside a
        # conditional expression does not carry into the `.get()` call, so the None case was
        # only ruled out by the reader, not the checker.
    ...
    return attach_stable_keys([_renumbered(new_id, slot) for new_id, slot in enumerate(order)])
