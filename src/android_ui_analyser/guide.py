...
SESSION_PROTOCOL: list[tuple[str, str]] = [
    ...
    (
        ...
        "`teardown.emulator_idle_stop_s` (20 minutes by default). An unattended harness that "
        "must leave no target running uses `aua session finish --stop-started-target`; the exact "
        "boot token is stopped, while a reused or pre-existing target is never stopped. Headless "
        "uses `-no-window` + "
        "**host GPU** on Mac — not "
...
KEY_FLAGS: list[tuple[str, str]] = [
    ...
    (
        ...
        "confidence,cost`), `--nonempty`, `--no-system`, `--no-ime`, `--no-wrappers`, `--all`, "
        "`--where-text <substr>`, "
...
def render_markdown(*, brief: bool = False) -> str:
    ...
    p.append(
        ...
        "outside the viewport do not satisfy presence checks, and iframe DOMs are merged into "
        "the viewport. `aua browser` adds console/network logs, cookies/web storage/cache, "
        "offline/throttle, scoped CORS, context proxy, HAR record/replay, request mocks, popup/"
    ...
    p.append(
        ...
        "scores the goal against what is on screen and decides each step itself, with no host "
        "round trip and no model. It is a word-overlap rule, so it reaches plainly-named "
        "destinations and hands back a reason when it cannot — `needs_host` for a goal only the "
        "host can do, `needs_auth` at a consent dialog, `no_progress` when the row it wants stops "
        "responding. Judging arrival stays with you: it reports what it tapped, not whether you "
        "are there. Prefer `goto` for anything you need to be reliable.\n"
    ...
    p.append(
        '                  "cost": {"avg_ms","max_ms","n"} } ],  // learned; absent with no history'
    )
    ...
    return {
        ...
        "schema_fields": {
            ...
            "element": [
                ...
                "source",
                "confidence",
                # What this control cost last time it was acted on, learned per (screen,
                # control): {"avg_ms", "max_ms", "n"}. Absent unless it has history.
                "cost",
            ],
            "meta": [
                "duration_ms",
