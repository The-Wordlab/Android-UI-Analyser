...
SESSION_PROTOCOL: list[tuple[str, str]] = [
    ...
    (
        ...
        "fails with `ConnectException` and no teardown will ever clean it up; `aua proxy status` "
        "(and `aua doctor`) is the only thing that will say so, and `aua proxy stop` un-points "
        "it. You should still `aua network restore` / `aua clock set --restore` "
