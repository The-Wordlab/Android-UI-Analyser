...
BRIEF_SESSION_PROTOCOL: list[tuple[str, str]] = [
    ...
    (
        ...
        "it; `--audio` enables emulator microphone injection), and `session finish` owns its "
        "cleanup.",
    ),
    (
...
- Start an emulator only with explicit scope (`session start --start-emulator`); use `--headed`
  ...
  macOS `mic speak`. Never repeat late-delivery errors; inspect their observation. On emulator
  36.4.10, `mic_repeat_unsafe` requires a restart.
