...
#: The pip extra that installs this provider's inference runtime (mlx-lm).
INSTALL_EXTRA = "functiongemma"
PROMPT_HANDOFF_FIELD = "handoff_candidate_id"
...
def _prompt_supports_handoff(prompt_schema: Mapping[str, Any]) -> bool:
    """Validate and return the provenance-bound handoff protocol capability."""

    value = prompt_schema.get(PROMPT_HANDOFF_FIELD)
    if value is None:
        return False
    if not isinstance(value, int) or isinstance(value, bool) or value != POLICY_HANDOFF_ID:
        raise ValueError("FunctionGemma prompt_schema handoff candidate ID is incompatible")
    return True
...
def _bundled_manifest(directory: Path) -> dict[str, Any]:
    ...
    try:
        ...
        _prompt_supports_handoff(prompt_schema)
    except ValueError as exc:
    ...
    *,
    ...
    if not bundled:
        ...
        if not configured_model.is_absolute():
            raise ValueError("adapter base model must be an absolute local path")
        # MLX records the training host's absolute snapshot path in adapter_config.json.
        # A separately pinned rollout manifest authenticates that exact config, weights,
        # and base-model digest, so requiring the historical directory to still exist
        # would make a reviewed adapter unusable after Pod cleanup or local transfer.
        # Unauthenticated adapters retain the strict existing-path contract.
        if not configured_model.is_dir() and manifest is None:
            raise ValueError("adapter base model must be an existing absolute local path")
        if configured_model.is_dir() and configured_model.resolve() != model:
    ...
    return {
        ...
        "adapter_declared_model_path": (str(adapter_config.get("model")) if not bundled else None),
        "adapter_declared_model_path_exists": (
            Path(str(adapter_config.get("model"))).is_dir() if not bundled else None
        ),
...
class FunctionGemmaPolicySelector(PolicyProvider):
    ...
    def _supported_candidate_counts(self) -> tuple[int, ...]:
        ...
        # Its own name rather than reusing `manifest` from the bundled branch above: that one is
        # a verified `dict`, this one is whatever the rollout capability happens to carry, and
        # sharing the name asserted the stronger shape for both.
        rollout_manifest = capability.get("manifest")
        if capability.get("authenticated") and isinstance(rollout_manifest, Mapping):
            prompt_schema = rollout_manifest.get("prompt_schema")
    ...
    def supports_handoff(self) -> bool:
        """Return whether an authenticated adapter learned the reserved handoff ID."""
        ...
        try:
            if _uses_bundled_adapter(self.settings):
                ...
                return _prompt_supports_handoff(manifest["prompt_schema"])
            capability = _explicit_rollout_capability(self.settings)
            explicit_manifest = capability.get("manifest")
            return bool(
                capability.get("authenticated")
                and isinstance(explicit_manifest, Mapping)
                and isinstance(explicit_manifest.get("prompt_schema"), Mapping)
                and _prompt_supports_handoff(explicit_manifest["prompt_schema"])
            )
        except Exception:
            return False
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            # Which pip extra supplies this provider's inference runtime. Declared here rather
            # than inferred from the reason string, so `aua doctor` can print the exact install
            # command without pattern-matching prose.
    ...
    def select(self, context: PolicyContext) -> int | None:
        ...
        if guarded != context.candidates:
            ...
            return None
        if context.allow_handoff and not self.supports_handoff():
            self.last_error = "the FunctionGemma adapter does not support policy handoff"
            return None
        ...
        try:
            ...
            if selected_id == POLICY_HANDOFF_ID and context.allow_handoff:
                pass
            elif selected_id not in offered_ids:
