...
Design notes
------------
- **Filed per app.** A flow lives at ``<memory.dir>/flows/<package>/<name>.yaml``, the
  package being the one it *starts* in (``app:``). Its steps still cross packages by
  design — an auth leg runs in a browser — so the directory records the owner rather than
  a boundary the journey may not leave. A flow declaring no app stays in ``flows/``
  itself, which is also where every flow written before this layout still lives and still
  loads. A name is therefore no longer a unique address: :meth:`FlowStore.resolve`
  documents how one is resolved and when it is refused.
...
from .memory import _LIFECYCLE_STEP_KINDS, REDACT_TOKENS, RouteStep, _safe
...
_KINDS = {
    ...
    # Cold-start setup: the things a flow could not do for itself.
...
def _prefs_values(raw: dict[Any, Any], *, index: int) -> dict[str, Any]:
    """Accept only the scalars a SharedPreferences entry can hold, keeping their YAML types.

    The type is the payload here: it decides whether the entry is written as ``<boolean>`` or
    ``<string>``, and the app's getter throws on the wrong one. So a container is refused
    rather than stringified, and ``null`` is refused rather than read as a deletion — nothing
    in this step removes a key, and guessing would remove app state the flow never named.
    """
    values: dict[str, Any] = {}
    for key, value in raw.items():
        ...
        raise _step_error(
            index,
            f"prefs_write value for {key!r} must be a string, number, or boolean, "
            f"got {type(value).__name__}",
            hint="String sets and key deletion are not part of this step.",
        )
    return values
...
def _parse_step(item: Any, index: int) -> RouteStep:
    ...
    if kind == "key" and isinstance(value, int) and not isinstance(value, bool):
        value = str(value)
    if kind == "wait-ms" and isinstance(value, int) and not isinstance(value, bool):
        # `wait_ms: 500` is the natural shorthand for a fixed delay; fold it into the same
        # mapping shape the dedicated branch below accepts (`{timeout_ms: 500}`) so there is
        # exactly one place that validates the duration.
        value = {"timeout_ms": value}
    if isinstance(value, str):
        if kind == "prefs-write":
            # Reached before the generic `_ARG_ALIAS` shortcut, which would otherwise accept
            # `prefs_write: settings.xml` and build a step that writes nothing.
            raise _step_error(
                index,
                "prefs_write needs a mapping: "
                "`prefs_write: {file: settings.xml, values: {key: value}}`",
            )
    ...
    if kind == "assert":
        ...
        # Not `_ARG_ALIAS` (that machinery is string-typed): the argument here is a duration.
        # Reuses `timeout_ms` — the field every wait-ish kind already carries — rather than
        # adding a new one; popped from `v` here so the generic `timeout_ms:` trailer below
        # sees nothing left to (harmlessly) re-apply.
        raw_ms = v.pop("timeout_ms", None)
        if raw_ms is None:
            raise _step_error(index, "wait_ms needs a `timeout_ms:` duration (or a bare integer)")
        kw["timeout_ms"] = _strict_int(raw_ms, field="wait_ms timeout_ms", index=index, minimum=1)
        ...
        # Structured because there is no single string that could carry it: the database and the
        # statement are both required and neither is a sensible default. Kept in `data` so the
        # assertion vocabulary that `goto` inspects is not widened by a write step.
        database = _string(v.pop("database", None), field="db_execute database", index=index)
        sql = _string(v.pop("sql", None), field="db_execute sql", index=index)
        if not database:
            raise _step_error(index, "db_execute needs a `database:` file name")
        if not sql:
            raise _step_error(index, "db_execute needs a `sql:` statement")
        payload: dict[str, Any] = {"database": database, "sql": sql}
        parameters = v.pop("parameters", None)
        if parameters is not None:
            if not isinstance(parameters, (dict, list)):
                raise _step_error(index, "db_execute `parameters:` must be a mapping or a list")
            payload["parameters"] = parameters
        restart = v.pop("restart", None)
        if restart is not None:
            if not isinstance(restart, bool):
                raise _step_error(index, "db_execute `restart:` must be true or false")
            payload["restart"] = restart
        kw["data"] = payload
        kw["arg"] = _string(v.pop("package", None), field="db_execute package", index=index)
    elif kind == "prefs-write":
        # `arg` is the prefs file, so the package rides on the generic `package:` extra below
        # and defaults to the flow's own app — the same shape `db_execute` would have if its
        # own argument were not already the package.
        raw_alias = v.pop("file", None)
        raw_arg = v.pop("arg", None)
        if raw_alias is not None and raw_arg is not None:
            raise _step_error(index, "prefs_write accepts `file:` or `arg:`, not both")
        name = _string(
            raw_alias if raw_alias is not None else raw_arg,
            field="prefs_write file",
            index=index,
            optional=False,
        )
        assert name is not None  # optional=False
        kw["arg"] = _prefs_file(name, index=index)
        raw_values = v.pop("values", None)
        if not isinstance(raw_values, dict) or not raw_values:
            raise _step_error(
                index,
                "prefs_write needs a non-empty `values:` mapping",
                hint="`values: {backend_env: staging, onboarding_seen: true}` — the YAML type "
                "picks the Android type (string / boolean / int / long / float).",
            )
        prefs_payload: dict[str, Any] = {"values": _prefs_values(raw_values, index=index)}
        if (relaunch := v.pop("relaunch", None)) is not None:
            prefs_payload["relaunch"] = _strict_bool(
                relaunch, field="prefs_write relaunch", index=index
            )
        kw["data"] = prefs_payload
    elif kind in ("launch-app", "stop-app", "clear-data"):
        # Optional arg: a bare `stop_app`/`launch_app`/`clear_data` targets the flow's own app.
...
def _render_step(s: RouteStep) -> dict[str, Any] | str:
    ...
    if s.kind == "db-execute":
        # Round-trips through `data`, so `flow show` prints something the parser accepts back.
        db_body: dict[str, Any] = {
            "database": s.data.get("database"),
            "sql": s.data.get("sql"),
        }
        if s.arg:
            db_body["package"] = s.arg
        for optional in ("parameters", "restart"):
            if optional in s.data:
                db_body[optional] = s.data[optional]
        return {"db_execute": db_body}
    if s.kind == "wait-ms":
        if s.timeout_ms is None:
            raise UsageError("wait_ms step needs a timeout_ms")
        if s.package:
            return {"wait_ms": {"timeout_ms": s.timeout_ms, "package": s.package}}
        return {"wait_ms": s.timeout_ms}
    if s.kind == "assert":
    ...
    if s.kind == "prefs-write":
        prefs_body: dict[str, Any] = {"file": s.arg, "values": dict(s.data.get("values") or {})}
        # Only when the author stated it: an omitted `relaunch:` and an explicit `relaunch: true`
        # behave alike, but rendering one as the other stops the step round-tripping.
        if "relaunch" in s.data:
            prefs_body["relaunch"] = bool(s.data["relaunch"])
        prefs_body.update(extras)
        return {key: prefs_body}
...
# Steps whose whole effect belongs to ANOTHER process and that report done the instant the
# request is accepted. ``open_link`` is a VIEW intent: ``am start`` returns when Android has
# *delivered* it, and there is no general signal for "the receiving app finished its side
# effects" — so the step can never be made to wait for its own outcome.
_HANDOFF_KINDS = frozenset({"open-link"})
# Steps that cannot run until the device answered: they match an element in a fresh hierarchy,
# wait for one, verify one, or (goto/flow/repeat/retry) run arbitrary steps that do. Composite
# and nested kinds count deliberately — treating them as opaque barriers keeps this check from
# refusing a flow whose observation lives one level down.
...
def _check_handoff_order(steps: list[RouteStep], *, base: int = 0) -> None:
    """Refuse a step order in which a hand-off's effect must be thrown away.

    Reproduced 2026-08-19: a reset flow ran ``open_link "<scheme>://set-flags?a=1&b=2"`` and
    then relaunched and force-stopped the app that received it. ``flow run`` answered
    ``ok=True`` — the open-link step "passed" in 1887ms — yet the shared-prefs file the handler
    writes was absent afterwards, twice, on clean runs; the same URI opened standalone wrote
    both values. The handler commits through ``SharedPreferences.edit {}``, i.e. ``apply()``,
    which flushes asynchronously, and a force-stop is a SIGKILL. Every step reported success,
    so nothing in the run pointed at the deeplink.

    aua cannot fix that by making ``open_link`` wait: it does not know which process claimed
    the URI, what that process decided to do, or when it finished — no such signal exists.
    What it *can* see, before the first device mutation, is the shape: a hand-off with nothing
    between it and the restart of the process it was given to. That flow cannot work, so it is
    refused here rather than discovered as a missing side effect three steps later.

    An intervening :data:`_OBSERVING_KINDS` step is accepted as sufficient. It is a settle, not
    a durability proof — the flow saw the receiver respond and Android ran the lifecycle
    transitions that flush pending ``apply()`` work, but arbitrary background work still cannot
    be proven done. ``flags_apply`` is the one path that does prove it: it reads the written
    values back off the device before restarting the app (see ``Engine.flags_set``).
    """
    for index, step in enumerate(steps):
        if step.kind not in _HANDOFF_KINDS:
            continue
        for offset, follower in enumerate(steps[index + 1 :], start=index + 1):
            if follower.kind in _OBSERVING_KINDS:
                break
            if follower.kind in _LIFECYCLE_STEP_KINDS:
                raise _step_error(
                    base + index,
                    f"`{_KEYS[step.kind]}` returns as soon as the intent is delivered, but step "
                    f"{base + offset + 1} (`{_KEYS[follower.kind]}`) restarts that process "
                    "before any step observed what the link did — whatever the receiver had not "
                    "finished is discarded, and both steps still report success",
                    hint=(
                        "Put a step that waits on the result between them (`wait_for`, "
                        "`assert_visible`, `assert`, or a bare `wait_stable`). For a set-flags "
                        "deeplink use `flags_apply`, which reads the written values back off "
                        "the device before it restarts the app."
                    ),
                )
...
def validate_resolved_steps(
    ...
    parameter with an intentionally empty default. Step *order* is judged here too, by
    :func:`_check_handoff_order` — a defect can belong to the sequence rather than to any one
    step, and the sequence is only fully known once params are materialized.
    """
    ...
    def validate(step: RouteStep, index: int) -> RouteStep:
        ...
        if step.substeps:
            _check_handoff_order(step.substeps, base=index * 100)
            update["substeps"] = [
    ...
    def fix(step: RouteStep) -> RouteStep:
        ...
        return step.model_copy(
            update={
                ...
                # `data` carries authored payloads (a prefs value, a SQL statement), so it takes
                # parameters like every other authored field; without this a `${ENV}` in one
                # reached the device verbatim.
                "data": sub_value(step.data),
...
def split_flow_ref(ref: str) -> tuple[str | None, str]:
    """Split an app-qualified flow reference — ``com.example.app:cold_start`` — into its parts.

    Two apps may each own a ``cold_start``, so a bare name is no longer a unique address and
    something has to be able to spell out which one is meant. The separator is a colon rather
    than the directory's own ``/`` because a nested ``flow:`` reference containing a slash is
    *by contract* a filesystem path (see :func:`looks_like_path`), and a package name can never
    contain a colon — so ``<package>:<name>`` stays unambiguous in every place a name is
    accepted: ``flow run``, ``flow show``, ``flow delete``, and a nested ``flow:`` step.
    """
    text = str(ref or "").strip()
    scope, sep, name = text.partition(FLOW_SCOPE_SEP)
...
class FlowStore:
    ...
    Only one level of subdirectory is indexed. Deeper collections (``flows/team/common/…``)
    stay reachable exactly as they already were — by path reference from a nested ``flow:``
    ...
    def app_dir(self, app: str | None) -> Path:
        ...
        root = self.flows_dir()
        return root / _safe(app) if app else root

    def files(self) -> list[Path]:
        """Every indexed flow file: the legacy flat layer first, then the per-app directories."""
        root = self.flows_dir()
        if not root.is_dir():
            return []
        return sorted(root.glob("*.yaml")) + sorted(root.glob("*/*.yaml"))

    def path(self, name: str, *, app: str | None = None) -> Path:
        """The one file that holds *(app, name)* — where :meth:`save` writes it.

        A flat ``flows/<name>.yaml`` that already declares this same app keeps its place:
        re-saving must update the file the user has rather than write a second copy under the
        package and leave the first one answering to the same name.
        """
        leaf = f"{_safe(name)}.yaml"
        if app:
            flat = self.flows_dir() / leaf
            if self._declared_app(flat) == app:
                return flat
        return self.app_dir(app) / leaf

    def find(self, ref: str) -> list[Path]:
        """Every stored file *ref* could mean. Empty means no such flow — never an error."""
        app, name = split_flow_ref(ref)
        leaf = f"{_safe(name)}.yaml"
        if app:
            namespaced = self.app_dir(app) / leaf
            flat = self.flows_dir() / leaf
            out = [namespaced]
            # A qualified reference has to keep working for a flow still in the flat layer.
            if flat != namespaced and self._declared_app(flat) == app:
                out.append(flat)
            return [path for path in out if path.is_file()]
        return [path for path in self.files() if path.name == leaf]

    def resolve(self, ref: str, *, referring_dir: Path | None = None) -> Path:
        """Turn a reference into exactly one file, or refuse and say which files it could mean.

        Names stopped being unique the moment flows were filed per app — two apps each owning a
        ``cold_start`` is the point of the layout — so resolution is a search with a deliberate
        order, and it never guesses:

        1. ``<package>:<name>`` addresses one app's flow. Nothing else is considered.
        2. A bare name inside a *composed* flow means the sibling filed next to the referring
           flow: a flow in ``flows/com.example.app/`` saying ``flow: sign_in`` means that app's
           ``sign_in``, not some other app's — and a flat flow composing another flat flow reads
           the same way, which is what keeps an existing pair working after some app introduces
           that name. Only flows filed together can be read as one another's siblings, so this
           step applies solely to a referring directory inside the library.
        3. Otherwise the whole library is searched, and a name held by more than one file is
           **refused** with every candidate named. Picking one — alphabetically first, or the
           foreground app's, or the newest — would replay a different journey than the caller
           asked for against a device it is mutating, and unlike a missing flow that is not
           something the caller can tell from the result.
        """
        app, name = split_flow_ref(ref)
        if app is None and referring_dir is not None:
            sibling = (Path(referring_dir).expanduser() / f"{_safe(name)}.yaml").resolve()
            root = self.flows_dir().resolve()
            if sibling.is_file() and root in (sibling.parent, sibling.parent.parent):
                return sibling
        matches = self.find(ref)
        if not matches:
            raise UsageError(f"no flow named '{ref}'", hint=f"known flows: {self._known()}")
        if len(matches) > 1:
            raise self._ambiguous(ref, matches)
        return matches[0]

    def reference(self, path: Path, counts: dict[str, int] | None = None) -> str | None:
        """The shortest reference that loads *path*, or None when none addresses it.

        A name only one file uses is its own reference; a name two apps share needs the
        ``<package>:<name>`` form. Two flat files cannot collide (one directory, one name), so
        the single unaddressable case is a flat legacy flow whose name a package directory also
        uses — surfaced as ``ref: null`` by ``flow list`` so it can be renamed or filed under
        its app, rather than quietly shadowed by whichever the search happened to see first.
        """
        counts = self._name_counts() if counts is None else counts
        if counts.get(path.stem, 0) <= 1:
            return path.stem
        if path.parent.resolve() != self.flows_dir().resolve():
            qualified = f"{path.parent.name}{FLOW_SCOPE_SEP}{path.stem}"
            if len(self.find(qualified)) == 1:
                return qualified
        return None
    ...
    def list(
        ...
        *,
        ...
        """Describe stored flows, optionally narrowed to the ones *app* can run.

        The *app* filter reads each flow's declared ``app:`` rather than the directory it sits
        in, so one query finds a namespaced flow and a legacy flat one alike. A flow declaring
        no app is listed for every *app* because it runs against every app; the entry's own
        ``app`` field is what tells the two apart.
        """
        counts = self._name_counts()
        out: list[dict[str, Any]] = []
        for p in self.files():
            try:
                flow = self.load_file(p)
            except Exception as exc:
                # An unreadable file declares nothing, so an app query cannot honestly exclude
                # it unless a different app's directory owns it.
                if app is None or p.parent in (self.flows_dir(), self.app_dir(app)):
                    out.append(
                        {
                            "name": p.stem,
                            "storage_name": p.stem,
                            "ref": self.reference(p, counts),
                            "error": str(exc),
                            "path": str(p),
                        }
                    )
                continue
            if app is not None and flow.app not in (None, app):
                continue
            ...
            out.append(
                {
                    ...
                    "storage_name": p.stem,
                    # …and once names repeat across apps, the storage name alone may not
                    # load either: `ref` is the spelling that does.
                    "ref": self.reference(p, counts),
                    "app": flow.app,
    ...
    def load_file(self, path: Path) -> Flow:
        """Parse one stored flow file; its file name is the flow's storage identity."""
        return parse_flow_yaml(Path(path).read_text(encoding="utf-8"), name=Path(path).stem)

    def load(self, ref: str, *, referring_dir: Path | None = None) -> Flow:
        return self.load_file(self.resolve(ref, referring_dir=referring_dir))

    def save(self, flow: Flow, *, force: bool = False) -> Path:
        """Write *flow* into its app's directory; existence is judged per app, not per name.

        Another package already owning a flow of this name is not a collision — that is what
        the layout is for — so ``force`` speaks only about the file at :meth:`path`. The cost is
        that the bare name now matches two files and has to be qualified; :meth:`resolve` says
        so instead of choosing.
        """
        check_saveable(flow)  # never write an artefact that cannot run
        path = self.path(flow.name, app=flow.app)
    ...
    def delete(self, ref: str) -> bool:
        """Delete one flow. Already absent is success; ambiguous is refused.

        Cleanup that has already happened must not fail a teardown, which is why an absent flow
        returns False rather than raising. A name two apps share is a different question:
        removing the wrong app's journey cannot be undone, so the caller is asked which.
        """
        matches = self.find(ref)
        if not matches:
            return False
    ...
    def _declared_app(self, path: Path) -> str | None:
        ...
        try:
            return self.load_file(path).app
        except (OSError, UsageError, ValueError):
            return None

    def _name_counts(self) -> dict[str, int]:
        counts: dict[str, int] = {}
        for path in self.files():
    ...
    def _known(self) -> str:
        counts = self._name_counts()
        refs = {self.reference(path, counts) or str(path) for path in self.files()}
        return ", ".join(sorted(refs)) or "(none)"

    # `Sequence`, not `list`: this class owns a method called `list`, which shadows the builtin
    # for every annotation declared after it.
    def _ambiguous(self, ref: str, matches: Sequence[Path]) -> UsageError:
        counts = self._name_counts()
        named = ", ".join(
            f"{self.reference(path, counts) or '(unaddressable)'} ({path})" for path in matches
        )
        return UsageError(
            f"flow name '{ref}' is ambiguous: {len(matches)} flows share it",
