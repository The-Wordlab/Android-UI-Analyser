...
A device change outlives the process that made it. An agent can be SIGKILLed, a daemon stopped, a
lease dropped — and lease expiry is *lazy*, so nothing runs at the moment a lease lapses. A change
whose undo exists only inside the mutating process is therefore lost exactly when it is needed, and
the person who finds out is the next agent, on a device that reports "Offline" or 401s every login.

So, if your feature changes state the device keeps after the command returns:

- Add it to `device_ledger.MUTATION_CATALOGUE` with an undo op (or `undo_op=None` plus a reason it
  needs none), and add the op to `device_ledger.UNDO_OPS` if it is new.
- Call `Engine.record_device_change(...)` **before** touching the device, and
  `Engine.forget_device_change(...)` when you undo it deliberately.
- Undo handlers are platform-neutral data replayed through the adapter — never native calls.
- `tests/test_every_device_mutation_registers_an_undo.py` fails until this is done, naming your
  call site.

Design and rationale: `docs/device-teardown.md`.
