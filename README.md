...
```bash
uvx --from \
  'android-ui-analyser[apple,rapidocr,audio] @ git+https://github.com/The-Wordlab/Android-UI-Analyser.git@v0.22.1' \
  aua --version
```

`uvx` creates an isolated environment and caches it for later calls. Replace `v0.22.1` with the
