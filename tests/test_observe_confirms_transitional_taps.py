...
This synthetic catalog fixture models a concrete unsafe sequence: tapping ``Catalog`` moves the
current page sideways, AUA returns while the old labels are still present plus one transient
unlabelled node, and the agent taps ``Catalog`` again. Dispatch succeeds both times; the bad
evidence is the first post-action observation.
...
HOME = _screen(
    _node("Catalog", rid=f"{PKG}:id/catalog", y=20, clickable=True),
    _node("Recent orders", y=120),
    _node("No recent orders", y=220),
)
...
CATALOG = _screen(
    _node("Catalog", rid=f"{PKG}:id/catalog", y=20, clickable=True),
    _node("Filter", rid=f"{PKG}:id/filter", y=120, clickable=True),
    _node("Featured products", y=220),
)


class DelayedCatalog(FakeDevice):
...
def test_layout_only_frame_is_confirmed_before_tap_returns(
    ...
    # Pin the synthetic trace's first settle result: AUA thought a fast hierarchy transition had
    ...
    out = eng.tap(selector={"text": "Catalog"}, observe=True)
...
def test_complete_destination_still_confirms_a_fast_settle(
    ...
    class ImmediateCatalog(DelayedCatalog):
        def click(self, x: int, y: int) -> None:
            super().click(x, y)
            self._xml = CATALOG

    dev = ImmediateCatalog()
    eng = _engine(tmp_path, dev)
    eng.analyze(source="hierarchy")
    monkeypatch.setattr(
        eng,
    ...
    assert out.observation is not None
    assert any(e.text == "Filter" for e in out.observation.elements)
    assert confirmations == [
        {"interval_ms": 80, "settle_ms": 350, "timeout_ms": 1400, "observe": False}
    ]
    assert out.settle is not None and out.settle["confirmation_ms"] >= 0
