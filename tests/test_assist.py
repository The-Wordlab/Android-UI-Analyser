...
def test_drive_guard_uses_resource_id_when_destructive_control_has_no_label(
    ...
    res = eng.analyze(source="hierarchy")
    target = next(
        element for element in res.elements if element.resource_id.endswith("deleteAccount")
    )
    _wire(
        monkeypatch,
        eng,
...
REPORTS = _hier(
    _node("android.widget.TextView", text="Build report", rid="x:id/h", b="[40,120][1040,210]"),
    _node("android.widget.Button", text="Run", rid="x:id/go", clk=True, b="[40,640][400,740]"),
)


def _to_reports_decider():
    """Tap 'Apps', then 'Reports', then declare done — a scripted 2-hop journey."""

    def fn(objective: str, elements: list[dict]) -> PlannerDecision:
        for target in ("Apps", "Reports"):
            for e in elements:
...
def test_navigate_drives_and_records_the_path(tmp_path, monkeypatch) -> None:
    dev = ScriptedDevice([HOME, APPS, REPORTS], package=P, serial="emu-n1")
    eng = _engine(tmp_path, dev, planner_enabled=True)
    _wire(monkeypatch, eng, StubPlanner(decide_fn=_to_reports_decider()))
    out = eng.navigate("open the image generator")
    assert out["ok"] is True and out["arrived"] is True, out
    assert sum(1 for c in dev.calls if c[0] == "click") == 2  # Apps + Reports
...
def test_navigate_until_stops_early(tmp_path, monkeypatch) -> None:
    # 'Run' is on the REPORTS screen; --until should stop as soon as it's visible.
    dev = ScriptedDevice(
        [HOME, APPS, REPORTS],
        package=P,
        serial="emu-n2",
    ...
    # text_index is only REPORTS' button; `has` finds it only once there.
    eng = _engine(tmp_path, dev, planner_enabled=True)
    _wire(monkeypatch, eng, StubPlanner(decide_fn=_to_reports_decider()))
    out = eng.navigate("reach report builder", until="Run")
    assert out["ok"] is True


def test_navigate_save_flow_writes_reusable_yaml(tmp_path, monkeypatch) -> None:
    dev = ScriptedDevice([HOME, APPS, REPORTS], package=P, serial="emu-n3")
    eng = _engine(tmp_path, dev, planner_enabled=True)
    _wire(monkeypatch, eng, StubPlanner(decide_fn=_to_reports_decider()))
    out = eng.navigate("open reports", save_flow="to_reports")
    assert out["ok"] and "flow_saved" in out
    flow = FlowStore(eng.config.memory).load("to_reports")
    assert flow.app == P
    labels = [s.label for s in flow.steps if s.kind == "tap"]
    assert "Apps" in labels and "Reports" in labels


def test_navigate_saved_flow_refreshes_same_engine_discovery_cache(tmp_path, monkeypatch) -> None:
    dev = ScriptedDevice([HOME, APPS, REPORTS], package=P, serial="emu-n3-cache")
    eng = _engine(tmp_path, dev, planner_enabled=True)
    _wire(monkeypatch, eng, StubPlanner(decide_fn=_to_reports_decider()))
    assert eng._flows_for(P) == []

    out = eng.navigate("open reports", save_flow="to_reports_cache")

    assert out["ok"] is True
    assert "to_reports_cache" in eng._flows_for(P)
