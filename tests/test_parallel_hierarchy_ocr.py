...
def test_hierarchy_and_apple_ocr_run_in_parallel_and_both_survive() -> None:
    ...
    #
    # The barrier IS the concurrency proof: each side blocks in it until the other arrives, so a
    # sequential implementation cannot get past the first `wait` and fails with
    # BrokenBarrierError after its 1s timeout. A wall-clock bound on top of that proved nothing
    # extra and failed on a loaded 2-core CI runner, where two 80ms sleeps that genuinely
    # overlapped still took 167ms end to end.
