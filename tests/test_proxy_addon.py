...
def test_glob_paths_and_hosts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert (
        _exchange(addon, path="/v1/abc/items", host="api.example.com").response.status_code == 204
    )
...
def test_rewrite_patches_the_real_response(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_rewrite_survives_a_non_json_body(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_flow_log_is_written_for_every_exchange(
    ...
    assert [(e["method"], e["path"], e["status"]) for e in entries] == [("POST", "/v1/chat", 200)]
    assert entries[0]["ts"] > 0


def test_flow_log_stays_free_of_bodies(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_binary_bodies_are_not_captured(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    addon = _load(monkeypatch, tmp_path)
