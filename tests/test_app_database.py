...
def test_query_no_restart_warning_says_the_app_was_left_stopped(tmp_path: Path) -> None:
    ...
    assert "warning" in result
    assert "not running" in result["warning"]


def test_schema_backup_execute_restore_all_warn_about_state_loss(tmp_path: Path) -> None:
    device = _device(tmp_path)
    cache = tmp_path / "cache"

    schema = app_database.database_schema(device, PKG, DB)
    assert "warning" in schema
    assert "navigat" in schema["warning"].lower()

    backup = app_database.backup_database(device, cache, PKG, DB)
    assert "warning" in backup

    executed = app_database.execute_database(
        device,
        cache,
        PKG,
        DB,
    ...
    assert "warning" in restored


# --------------------------------------------------------------------- non-destructive `live` read


def test_query_live_reads_without_stopping_the_app_or_touching_shm(tmp_path: Path) -> None:
    device = _device(tmp_path)

    result = app_database.query_database(
        device, PKG, DB, "SELECT id, value FROM children ORDER BY id", live=True
    )

    assert result["rows"] == [[10, "before"]]
    assert not [call for call in device.calls if call[0] in ("stop_app", "launch_app")]
    read_paths = {call[1][1] for call in device.calls if call[0] == "read_app_file"}
    assert f"databases/{DB}" in read_paths
    assert f"databases/{DB}-wal" in read_paths
    assert f"databases/{DB}-shm" not in read_paths
    assert result["coherent"] is False
    assert result["app_stopped"] is False
    assert result["app_restarted"] is False
    assert "warning" in result
    assert "torn" in result["warning"].lower()


def test_query_live_refuses_mutating_sql(tmp_path: Path) -> None:
    device = _device(tmp_path)
    original = dict(device.app_files)

    with pytest.raises(UsageError, match="query failed"):
        app_database.query_database(
            device, PKG, DB, "UPDATE children SET value = 'bad'", live=True
        )
...
def test_query_live_survives_a_wal_truncated_mid_copy(tmp_path: Path) -> None:
    """A torn WAL must fail safe (stale-but-consistent), never return corrupt rows."""
    device = _device(tmp_path)
    grow_path = tmp_path / "grow.db"
    grow_path.write_bytes(device.app_files[f"databases/{DB}"])
    wal_path = grow_path.with_name(f"{grow_path.name}-wal")
    wal_path.write_bytes(device.app_files[f"databases/{DB}-wal"])
    connection = sqlite3.connect(grow_path)
    try:
        connection.execute("PRAGMA journal_mode=WAL")
        connection.execute("PRAGMA wal_autocheckpoint=0")
        connection.execute("UPDATE children SET value = 'after' WHERE id = 10")
        connection.commit()
        # Capture both files while still open: with WAL mode, the main file is untouched by
        # the write until a checkpoint runs, and closing the last connection checkpoints (and
        # can even remove) the WAL -- exactly the moment this test needs to freeze before.
        main_bytes = grow_path.read_bytes()
        full_wal = wal_path.read_bytes()
    finally:
        connection.close()
    device.app_files[f"databases/{DB}"] = main_bytes
    # Simulate a copy that caught the newest WAL frame mid-write.
    device.app_files[f"databases/{DB}-wal"] = full_wal[:-5]

    result = app_database.query_database(
        device, PKG, DB, "SELECT value FROM children WHERE id = 10", live=True
    )

    assert result["rows"] == [["before"]]


def test_execute_backup_restore_have_no_live_bypass() -> None:
    import inspect

    assert "live" not in inspect.signature(app_database.execute_database).parameters
    assert "live" not in inspect.signature(app_database.backup_database).parameters
