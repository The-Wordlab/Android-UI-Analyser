...
A goal is a phrase — "apps search", "create account" — and the map holds those words scattered
across a screen's name, its anchors, its key-element labels and the routes that reach it. Matching
the query whole meant `--find "search"` and `--find "apps"` each answered while `--find "apps
search"` reported "no matching screen in memory" about a map that held the route.

Measured 2026-08-10: an agent was shown `aua map --find "<goal>"` in its very first analyze, on a
map of 135 screens and 613 routes, and navigated the whole task by tapping instead.
...
the screens it passes through, so without that split a chat screen reached *via* an Apps tab
outranks the Apps screen itself.
...
def test_a_goal_phrase_finds_something_at_all() -> None:
    ...
    assert _find_targets(_app(), "no apps") != []


def test_the_terms_do_not_have_to_be_adjacent() -> None:
    found = _find_targets(_app(), "apps create")

    assert "search_results" in found, "'create app' and 'no apps found' are both on that screen"


def test_a_term_that_is_nowhere_still_matches_nothing() -> None:
    assert _find_targets(_app(), "apps zzzqqqxyz") == []
