...
def _observation(*, title: str = "Recent items", loading: bool = False) -> AnalyzeResult:
    elements = [
        ...
        Element(
            ...
            type="Button",
            text="Cached example",
            resource_id=f"{PKG}:id/threadPreviewItem",
...
def test_same_activity_wrong_predicate_returns_bounded_arrival_mismatch(
    ...
    result = engine.await_predicate(
        "text:Creation detail,!text:Loading",
        timeout_ms=600_000,
        poll_ms=1,
        rich_ui=False,
        observe=True,
    ...
    assert result.arrival_mismatch == {
        ...
        "original_predicate": "text:Creation detail,!text:Loading",
        "unmet_positive_terms": ["text:Creation detail"],
        "suggested_positive_predicates": [
            "rid:threadPreviewItem",
            "rid:recentTitle",
            "text:Cached example",
        ],
        "stable_checks": 2,
        ...
        "recommended_call": (
            "aua await-and-analyze 'rid:threadPreviewItem,!text:Loading' --observe"
        ),
        "recommended_mcp_call": {
            "tool": "await_and_analyze",
            "arguments": {"predicate": "rid:threadPreviewItem,!text:Loading"},
        },
    }
...
def test_correct_action_predicate_succeeds_without_mismatch_sampling(
    ...
    device = FakeDevice(
        ...
        resource_index={"threadPreviewItem": (20, 220, 900, 360)},
        text_index={},
    )
    engine = _engine(tmp_path, device)
    _seed_action_baseline(engine)
