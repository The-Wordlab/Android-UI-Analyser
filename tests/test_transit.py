"""Cross-package transit (PRD §6b): the Google-auth leg becomes ONE replayable edge.

The live failure this covers: tapping "Continue with Google" opens a Chrome custom tab
(different package), so route edges were never recorded across the auth leg and `goto
...
CHROME_PICKER = _hier(
    ...
    _node("android.view.View", text="Engineering Team", clk=True, b="[40,300][1040,400]", pkg=CHROME),
    _node("android.view.View", text="Use another account", clk=True, b="[40,440][1040,540]", pkg=CHROME),
)
...
OTHER_APP = _hier(
    ...
    _node("android.widget.Button", text="Equals", clk=True, b="[40,300][400,400]", pkg="com.other.calc"),
)
...
def test_goto_hands_off_on_redacted_transit_step(tmp_path: Path) -> None:
    ...
    store.record_route(
        ...
        "onboarding",
        steps=[
            RouteStep(kind="tap", label="Continue with Google", resource_id="google"),
