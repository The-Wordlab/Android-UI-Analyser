...
Each case mirrors a plausible wrong guess made by an agent driving an app. The pattern is the
same: one command works on the first try, which reads as "I know
...
def _action_payload() -> dict:
    return {
        ...
        "change": {
            ...
            "text_added": ["Create your account", "Continue with Example ID"],
            "text_removed": ["No products found"],
        },
        "next_actions": [{"id": 2, "label": "Continue with Example ID"}],
        "observation": {
            "schema_version": 1,
            "screen": {"width": 1080, "height": 2400, "package": "com.example.app"},
            "elements": [
                {"id": 2, "text": "Continue with Example ID", "clickable": True, "window": "app"},
...
def test_an_action_renders_its_elements_as_tsv_rows() -> None:
    ...
    assert any("Continue with Example ID" in row for row in rows[1:])
