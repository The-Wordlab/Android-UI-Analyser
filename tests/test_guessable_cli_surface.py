...
- ``screen`` is what gets typed instead of ``analyze``, so it is an alias.
- ``input --text "foo"`` is assumed from ``--rid``/``--desc`` being flags; the text is positional,
  and ``--text`` cannot be reused because it selects by label on every other command. It is
  accepted purely so the error can say that.
...
def test_screen_is_an_alias_for_analyze() -> None:
    names = _command_names()
    assert "screen" in names, "an agent that has not read the guide types `screen` first"
    assert "analyze" in names, "and the documented name keeps working"


def test_screen_offers_exactly_what_analyze_offers() -> None:
    """Typer wraps each registration separately, so compare the surface, not the callback."""
    from typer.main import get_command

    commands = get_command(app).commands
    assert commands["screen"].callback.__name__ == commands["analyze"].callback.__name__
    opts = {p.name for p in commands["screen"].params}
    assert opts == {p.name for p in commands["analyze"].params}, "an alias must not drift"


def test_input_text_is_answered_not_just_rejected() -> None:
    from typer.testing import CliRunner

    result = CliRunner().invoke(app, ["input", "--rid", "promptField", "--text", "hello"])
