...
def test_the_always_on_capture_buffer_does_not_block_the_restart(tmp_path: Path) -> None:
    ...
    `serve` starts a buffer on every warm daemon, so it matched every daemon there is —
    thirteen consecutive calls in one session each warned and each paid the full device attach.
    It protected nothing retrievable either: under skew every `capture_*` command is in
    `_DAEMON_ONLY_METHODS` and raises rather than answering, so those frames stay unreadable
