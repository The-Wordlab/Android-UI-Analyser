...
def test_auto_detected_flag_context_change_names_its_detection_source(tmp_path: Path) -> None:
    ...
    """
    store = _store(tmp_path)
    serial = "fictional-auto-detected-boundary"
    store.observe_screen(serial, package=P, elements=_elements(HOME), screen_height=800)
    store.observe_action(
        serial,
        RouteStep(kind="tap", resource_id="settingsRow", by="id", package=P),
    )

    store.activate_flag_context(
        serial,
        P,
        {"catalog_variant": "b"},
        verified=True,
    ...
    store.observe_action(
        serial,
        RouteStep(kind="tap", resource_id="languageRow", by="id", package=P),
    )
