...
def _pin_the_loaded_source(monkeypatch: pytest.MonkeyPatch) -> None:
    """These tests are about the idle policy, not about code identity.

    `_idle_tick` also retires a daemon whose loaded source no longer matches the tree, which is
    deliberately sensitive to any edit under the package. That makes it fire for real during a
    suite run that races a file write — correct in production, meaningless here — so the two
    are separated: pin identity, exercise the policy.
    """
