...
A synthetic long-running export demonstrates the ambiguity. With no way to wait on a *condition*,
a caller can only poll, wait fixed intervals, or conclude "stuck" from a stale frame, and nothing
in the output distinguishes that from a slow backend.
...
**No `await-network-idle`.** A sample app may prefetch, post telemetry, or stream status updates
continuously, so idleness would be a flaky proxy for "this is ready".
...
@pytest.mark.parametrize(
    "field,expected", [("text", "text"), ("rid", "rid"), ("id", "rid"), ("desc", "desc")]
)
...
def test_screen_changed_returns_early_rather_than_spending_the_budget(tmp_path: Path) -> None:
    ...
    dev = ScriptedScreens(frames=[{"Generating"}], activities=[".EditorActivity", ".SomewhereElse"])
    eng = _engine(tmp_path, dev)
