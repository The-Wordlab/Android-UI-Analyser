...
def test_parse_flow_yaml_shorthand_and_mapping() -> None:
    ...
    assert flow.name == "open_reports" and flow.app == P
...
def test_resolve_params_defaults_and_overrides() -> None:
    ...
    assert steps[2].label == "Reports"  # declared default applies
    steps = resolve_params(flow, {"TOOL": "Calendar"})
    assert steps[2].label == "Calendar"  # override wins
...
def test_flow_run_from_file_drives_whole_journey(tmp_path: Path) -> None:
    flow_file = tmp_path / "to_reports.yaml"
    flow_file.write_text(_reports_flow_text(), encoding="utf-8")
    dev = ScriptedDevice(
        [HOME, APPS, REPORTS],
        package=P,
        serial="emu-flow",
        text_index={"Run": (40, 640, 400, 740)},
    )
    eng = _engine(tmp_path, dev)
    out = eng.flow_run(file=str(flow_file))
    assert out["ok"] is True, out
    assert out["flow"] == "to_reports"
...
def test_playbook_notes_recipes_description(tmp_path) -> None:
    ...
    store.remember_note(P, "seasonal banner needs a feature flag")
    store.remember_recipe(P, "login_full", "tap 'Use demo account'")
    store.remember_recipe(P, "login_full", "tap demoAccountButton")  # updates in place
    app = store.load(P)
    assert app.description == "Example App (dev build)"
    assert app.notes == ["seasonal banner needs a feature flag"]
    assert len(app.recipes) == 1 and app.recipes[0].note == "tap demoAccountButton"
...
def test_flow_open_link_and_bare_stop_app(tmp_path) -> None:
    # The set-feature-flags recipe: open a deeplink, let the receiver answer, restart the app.
    # The settle before the restart is not decoration — a deeplink returns on delivery, and
    # force-stopping the receiver first discards whatever it had not committed (see
    # tests/test_flow_deeplink_handoff_order.py).
    text = """
name: set_flags
app: com.example.app
steps:
  - open_link: "myapp://set-flags?catalog_layout=variant_b"
  - wait_stable
  - stop_app
  - launch_app
  - wait_stable
"""
    ...
    kinds = [s.kind for s in flow.steps]
    assert kinds == ["open-link", "wait-stable", "stop-app", "launch-app", "wait-stable"]
    assert flow.steps[0].arg == "myapp://set-flags?catalog_layout=variant_b"
    assert flow.steps[2].arg is None  # bare stop_app → defaults to flow.app at run
    ...
    assert (
        "open_link",
        ("myapp://set-flags?catalog_layout=variant_b", "com.example.app"),
    ) in dev.calls
...
def test_render_map_shows_playbook(tmp_path) -> None:
    ...
    store.remember_note(P, "the Catalog tab is catalogTab (Browse=Catalog)")
...
def test_orient_surfaces_playbook(tmp_path) -> None:
    ...
    store.set_description(P, "Example App")
    store.remember_recipe(P, "login_full", "tap demoAccountButton")
    ...
    assert out["recipes"]["login_full"] == "tap demoAccountButton"
...
def test_cli_remember_and_about(tmp_path, monkeypatch) -> None:
    ...
    r = runner.invoke(
        app,
        ["remember", "--app", P, "--recipe", "login_full", "--note", "tap demoAccountButton"],
    )
    ...
    assert data["recipes"]["login_full"] == "tap demoAccountButton"
...
def test_flow_composition_runs_sub_flow(tmp_path) -> None:
    ...
    dev = ScriptedDevice(
        [HOME, APPS], package=P, serial="emu-compose", text_index={"Reports": (0, 0, 9, 9)}
    )
    eng = Engine(cfg, device=dev, factory=ProviderFactory(cfg))
...
def test_flow_assert_visible_by_id(tmp_path) -> None:
    text = (
        ...
        "  - wait_for: {id: queryField, timeout_ms: 2000}\n"
    )
    ...
    dev = FakeDevice(
        hierarchy_xml=HOME,
        package=P,
        resource_index={"x:id/containerDetail": (0, 0, 9, 9), "x:id/queryField": (0, 0, 9, 9)},
    )
    eng = Engine(cfg, device=dev, factory=ProviderFactory(cfg))
    f = tmp_path / "t.yaml"
