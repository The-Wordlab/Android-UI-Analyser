...
from test_navigation import REPORTS, ScriptedDevice
...
def test_goto_from_here_skips_matched_prefix(tmp_path) -> None:
    """Multi-step edge; already on Apps → --from-here resumes at tap Reports."""
    ...
    store.record_route(
        ...
        steps=[
            ...
            RouteStep(kind="tap", label="Reports", resource_id="tool_reports"),
        ],
    )
    # Mid-edge: session still says "home", but the device already shows Apps.
    device = ScriptedDevice([APPS, REPORTS], package=P, serial="from-here")
    store.save_session(
        "from-here",
        store.load_session("from-here").model_copy(update={"package": P, "current_screen": "home"}),
    )
    engine = Engine(cfg, device=device)

    clicks_before = len([c for c in device.calls if c[0] == "click"])
    result = engine.goto("reports", from_here=True)
    clicks_after = len([c for c in device.calls if c[0] == "click"])

    assert result["ok"] is True
    assert result.get("arrived") is True
    assert clicks_after - clicks_before == 1  # only Reports — Apps already done
