...
def _validate_row(
    ...
    *,
    ...
    # FunctionGemma carries its activation on a "developer" turn. Some bases — Qwen3.5 among
    # them — ship a chat template that accepts only system/user/assistant/tool and raises on any
    # other role, so a corpus rendered for those bases carries the identical activation text on a
    # "system" turn instead. The role is a transport detail of the base; the activation text, the
    # single function declaration, and the target are the contract, and those are checked below
    # unchanged. A run must state which rendering it expects rather than accepting either.
    if [message.get("role") for message in messages] != [activation_role, "user", "assistant"]:
        raise ValueError(f"{split}: roles must be {activation_role}, user, assistant")
    ...
    if state.get("handoff") == {
        ...
        by_id[POLICY_HANDOFF_ID] = {
            "id": POLICY_HANDOFF_ID,
            "call": {
                "tool": "policy_handoff",
                "arguments": {"reason": "no_supplied_candidate_advances_goal"},
            },
            "risk": "safe",
            "authorized": True,
            "redundant": False,
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for split in SPLITS:
        ...
        records = [
            _validate_row(
                ...
                activation_role=getattr(args, "activation_role", "developer"),
            )
            for row in rows
        ]
    ...
    # Balance is only meaningful *within* a candidate count. A two-candidate case can never
    # place its answer at id 2 or 3, so pooling every cardinality makes the high ids look
    # starved and rejects a corpus that is in fact uniform at every size. Judge each size
    # against the ids that size can actually use.
    by_cardinality: dict[int, Counter[int]] = {}
    for record in all_records:
        if record["target_id"] == POLICY_HANDOFF_ID:
            continue
        by_cardinality.setdefault(int(record["cardinality"]), Counter())[record["target_id"]] += 1
    if not by_cardinality:
        raise ValueError("no opaque target positions to validate")
    for cardinality, counts in sorted(by_cardinality.items()):
        if len(counts) < cardinality:
            raise ValueError(
                f"target positions are not sufficiently randomized at cardinality "
                f"{cardinality}: {dict(counts)}"
            )
        maximum = max(counts.values())
        minimum = min(counts.values())
        if maximum > minimum * 1.75:
            raise ValueError(
                f"opaque target-position distribution is too imbalanced at cardinality "
                f"{cardinality}: {dict(counts)}"
            )
    ...
    result = {
        ...
        "handoff_targets": target_positions[POLICY_HANDOFF_ID],
        "splits": split_stats,
    }
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="developer",
        help="Turn carrying the activation. Bases whose chat template lacks a developer role "
        "use system; the activation text itself is unchanged.",
    )
