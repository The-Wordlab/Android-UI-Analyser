...
def _candidate_map(state: dict[str, Any]) -> dict[int, dict[str, Any]]:
    """Return offered candidates plus the authenticated non-executing handoff sentinel."""
...
def _metrics(rows: list[dict[str, Any]], predictions: list[Prediction]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        candidates = list(_candidate_map(state).values())
        eligible = [
            candidate
            for candidate in candidates
...
def _hf_snapshot_identity(value: str) -> tuple[str, str] | None:
    """Return the immutable Hugging Face cache identity for a snapshot root."""
    parts = Path(value).parts
    try:
        snapshots_index = parts.index("snapshots")
    except ValueError:
        return None
    if snapshots_index < 1 or snapshots_index + 2 != len(parts):
        return None
    repository = parts[snapshots_index - 1]
    revision = parts[snapshots_index + 1]
    if not repository.startswith("models--") or not revision:
        return None
...
def _validate_adapter(model: str, adapter: Path | None) -> dict[str, Any] | None:
    ...
    configured_snapshot = _hf_snapshot_identity(configured_model)
    requested_snapshot = _hf_snapshot_identity(model)
    if configured_snapshot is not None and configured_snapshot == requested_snapshot:
        matches = True
    if not matches:
        raise ValueError(
