...
# Every group above only SHARES a token ("Saved articles" / "Saved lessons"); no label ever
# contains another. That left the real trap untested: a distractor whose label strictly CONTAINS
# the requested destination. Measured 2026-08-18, the deterministic recommendation took the longer
# label in 6 of 24 rotations, and the same relation had beaten every FunctionGemma version live.
# (target, a label that strictly contains it, two controls sharing the target's head noun)
_CONTAINMENT: tuple[tuple[str, str, str, str], ...] = (
    ("River studio", "River studio archive", "Summit studio", "Timber studio"),
    ("Kite library", "Kite library settings", "Jasper library", "Lagoon library"),
    ("Amber center", "Amber center archive", "Birch center", "Coral center"),
    ("Garden index", "Garden index archive", "Forest index", "Harbor index"),
    ("Orchid records", "Orchid records archive", "North records", "Prairie records"),
)
...
def benchmark_cases() -> list[BenchmarkCase]:
    ...
    # A label that IS the destination must outrank one that merely contains it, in every frame
    # order. Ranking ties used to fall through to element id, so the winner was whichever of the
    # two came first.
    for group_index, (target, contains, *siblings) in enumerate(_CONTAINMENT):
        labels = [target, contains, *siblings]
        for rotation in range(len(labels)):
            ordered = [*labels[rotation:], *labels[:rotation]]
            case_id = f"contains-g{group_index:02d}-r{rotation}"
            elements: list[Element] = []
            calls: dict[str, dict[str, Any]] = {}
            for element_index, label in enumerate(ordered, start=1):
                element, call = _control(
                    element_index,
                    label,
                    _SUMMARIES[element_index % len(_SUMMARIES)],
                    representation="rid",
                )
                elements.append(element)
                calls[label] = call
            cases.append(
                BenchmarkCase(
                    case_id=case_id,
                    family="exact_beats_containment",
                    goal=f"Open {target}",
                    target=target,
                    observation=_observation(case_id, elements),
                    oracle_call=calls[target],
