...
- _observed actions (`*_and_analyze` MCP / `*-and-analyze` CLI)_: return the post-action screen inline (`observation`, fresh ids), and the explicit names cannot disable that readback. Prefer `hide-keyboard-and-analyze` over `key-and-analyze back` when the IME is covering the tree. For nested navigation use `back-until-and-analyze '<destination>'`; pass `--back-id <fresh-id>` only when the first app-owned Back icon is unlabeled
