...
2. **Start the warm daemon.** `aua daemon start` — holds the device connection + loaded models warm so each later call is ~tens of ms instead of paying Python/connect startup. Optional; every command still works without it.
