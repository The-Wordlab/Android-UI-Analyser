...
- _location / orientation / airplane / network / media / record / clock_: `location set LAT,LON`, `orientation set|get`, `airplane on|off|toggle`, `network status|offline --verify|restore` (offline mode is saved, verified, and reversible), `media add PATH`, `record start|stop PATH`, `clock set --ms <unix-ms>` / `clock restore` (time travel invalidates auth — always restore)
